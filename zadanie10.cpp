...
/*
    ...
    PRIKLAD:
        na obrazku
*/
list<char> breadthFirstSearchUpperCasesHelper(const Node *node) {
    list<char> answer;
    if (node == nullptr) {
        return answer;
    }
...
list<char> breadthFirstSearchUpperCases(const Tree *tree) {
    return breadthFirstSearchUpperCasesHelper(tree->root);
}
...
City *findCity(Planet *planet, const string &name) {
    auto it = find_if(planet->cities.begin(), planet->cities.end(), [&name](City &c) {
        return c.name == name;
    });
...
list<string> breadthFirstSearchReachable(Planet *planet, const string &startCity) {
    ...
    City *start = findCity(planet, startCity);
    if (start == nullptr) {
...
map<string, unsigned> dijkstra(Planet *planet, const string &startCity) {
    ...
    start->searchData.foundShortestPath = true;

    auto compare = [](const City *a, const City *b) {
        return a->searchData.distance > b->searchData.distance;
    };
    priority_queue<City *, vector<City *>, decltype(compare)> pq(compare);
    pq.push(start);
    ...
    while (!pq.empty()) {
        ...
        for (RoadTo &road: current->roads) {
            ...
            unsigned newDist = current->searchData.distance + road.length;
            if (newDist < neighbor->searchData.distance) {
                neighbor->searchData.distance = newDist;
                neighbor->searchData.previous = current;
                pq.push(neighbor);
            }
        }
    }

    for (const City &city: planet->cities) {
        if (city.searchData.distance != numeric_limits<unsigned>::max()) {
            distances[city.name] = city.searchData.distance;
        }
    }
