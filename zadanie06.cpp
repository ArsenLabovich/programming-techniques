...
public:
    ...
    char getChar(size_t index) const{
        if(data == nullptr || index < 0 || index >= strlen(data)){return '\0';}
        return data[index];
    }
    ...
    void append(const char *text){
        ...
        size_t newLength = strlen(data) + strlen(text);
...
void e(char *p,char *s,int a,int b,int i){for(;i<a;)cout<<(char)('A'+((p[i]+1+s[i++%b]-'A')-'A')%26);}
int main() {
long  i = 0;
    while(1){
        i+=1;
