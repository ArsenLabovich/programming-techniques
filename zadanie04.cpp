...
void bubbleSort(Weight *data, const size_t length) {
    ...
    for (size_t i = 0; i < length - 1; i++) {
        if (data[i].packing + data[i].product < data[i + 1].packing + data[i+1].product) {
            Weight temp = data[i];
            data[i] = data[i + 1];
            data[i + 1] = temp;
            i = -1;
...
size_t getPivotIndex(const int *data, const size_t low, const size_t high) {
    int arr[] = {data[low], data[high - 1], data[(low + high) / 2]};
    bubbleSort(arr, 3);
    if (arr[1] == data[low]) {
        return low;
    } else if (arr[1] == data[high - 1]) {
        return high - 1;
    } else {
        return (low + high) / 2;
    }
...
size_t partition(int *data, const size_t pivot, const size_t low, const size_t high) {
    ...
    swap(&data[insert_index], &data[high - 1]);
    return insert_index;
}
